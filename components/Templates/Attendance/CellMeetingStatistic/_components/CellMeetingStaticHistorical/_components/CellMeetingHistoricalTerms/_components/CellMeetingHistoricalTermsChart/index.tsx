import {BarChart, LineChart} from "@tremor/react";
import {TCellMeetingTermStatic} from "../../../../../../../../../../firebase/CellMeeting/CellMeetingStatic";
import {convertTerm} from "../../../../../../../../../../utils/utils";

type Props = {
  data: TCellMeetingTermStatic[];
};

const customBarTooltip = (props: any) => {
  const {payload, active, label} = props;
  if (!active || !payload) return null;
  return (
    <div className="w-56 rounded-tremor-default border border-tremor-border py-2 bg-tremor-background text-tremor-default shadow-tremor-dropdown">
      <div className="pb-2 border-b px-2">
        <p>{label}</p>
      </div>
      {payload.map((category: any, idx: number) => (
        <div key={idx} className="flex items-center px-2 pt-2 space-x-2.5">
          <div className={`w-2 h-2 bg-${category.color}-500 rounded`} />
          <p className="text-tremor-content">
            {category.dataKey === "totalAvg"
              ? "총원"
              : category.dataKey === "attendanceAvg"
              ? "출석"
              : "미참석"}
          </p>
          <p className="font-medium text-tremor-content-emphasis">
            {category.value} 명
          </p>
        </div>
      ))}
    </div>
  );
};

const customLineTooltip = (props: any) => {
  const {payload, active, label} = props;
  if (!active || !payload) return null;
  return (
    <div className="w-56 rounded-tremor-default border border-tremor-border py-2 bg-tremor-background text-tremor-default shadow-tremor-dropdown">
      <div className="pb-2 border-b px-2">
        <p>{label}</p>
      </div>
      {payload.map((category: any, idx: number) => (
        <div key={idx} className="flex items-center px-2 pt-2 space-x-2.5">
          <div className={`w-2 h-2 bg-${category.color}-500 rounded`} />
          <p className="text-tremor-content">출석률</p>
          <p className="font-medium text-tremor-content-emphasis">
            {category.value} %
          </p>
        </div>
      ))}
    </div>
  );
};

const CellMeetingHistoricalTermsChart = ({data}: Props) => {
  const formattedData = data.map((item) => ({
    ...item,
    term: convertTerm(item.term),
  }));

  return (
    <div>
      <div className="pt-5">
        <div className="flex justify-end mb-2">
          <div className="flex items-center space-x-4 text-xs">
            <div className="flex items-center space-x-1">
              <span className="w-2 h-2 bg-rose-500 rounded-full"></span>
              <span>미참석인원</span>
            </div>
            <div className="flex items-center space-x-1">
              <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
              <span>출석인원</span>
            </div>
            <div className="flex items-center space-x-1">
              <span className="w-2 h-2 bg-teal-500 rounded-full"></span>
              <span>전체인원</span>
            </div>
          </div>
        </div>
        <BarChart
          data={formattedData}
          index="term"
          categories={["absentAvg", "attendanceAvg", "totalAvg"]}
          colors={["rose", "blue", "teal"]}
          yAxisWidth={30}
          // onValueChange={(v) => setValue(v)}
          intervalType={"preserveStartEnd"}
          className="mt-4 h-72 text-sm w-full"
          customTooltip={customBarTooltip}
          showTooltip
          showLegend={false}
        />
      </div>
      <div className="mt-10">
        <div className="flex justify-end mb-2">
          <div className="flex items-center space-x-4 text-xs">
            <div className="flex items-center space-x-1">
              <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
              <span>출석률</span>
            </div>
          </div>
        </div>
        <LineChart
          data={formattedData}
          index="term"
          categories={["attendanceRateAvg"]}
          colors={["blue"]}
          yAxisWidth={30}
          // onValueChange={(v) => setValue(v)}
          intervalType={"preserveStartEnd"}
          className="mt-4 h-72 text-sm w-full"
          customTooltip={customLineTooltip}
          showTooltip
          showLegend={false}
        />
      </div>
    </div>
  );
};

export default CellMeetingHistoricalTermsChart;
