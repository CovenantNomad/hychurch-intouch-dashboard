...
interface CellDallantListItemProps {
  ...
  cellName: string;
  totalAmount: number;
  cellMembers: CellDallantMemberType[]
}

const CellDallantListItem = ({ cellId, cellName, cellMembers, totalAmount }: CellDallantListItemProps) => {
  const participants = cellMembers.filter(member => member.totalAmount !== 0)
  return (
    <Link 
      href={{
        ...
        <dl className="-my-3 divide-y divide-gray-100 px-6 py-4 text-sm leading-6 bg-gray-50">
          <div className="flex justify-between gap-x-4 py-3">
            ...
            <dd className="text-gray-700">
              <p>{participants.length}명 참여 중</p>
            </dd>
          </div>
          <div className="flex justify-between gap-x-4 py-3">
