...
const CellMeetingCellStaticSection =
  ({}: CellMeetingCellStaticSectionProps) => {
        ...
        <div className="py-4">
          ...
          >
            <DateRangePickerItem
              ...
              value="thisMonth"
              from={
                new Date(today.getFullYear(), today.getMonth() - 1, 1, 0, 0)
              }
              to={new Date(today.getFullYear(), today.getMonth(), 0, 23, 59)}
            >
              이번달
            ...
            <DateRangePickerItem
              ...
              value="previousMonth"
              from={
                new Date(today.getFullYear(), today.getMonth() - 2, 1, 23, 59)
              }
              to={
                new Date(today.getFullYear(), today.getMonth() - 1, 0, 23, 59)
              }
            >
              지난한달
            </DateRangePickerItem>
            ...
            <tbody className="divide-y divide-gray-200">
              {orderedCellList ? (
                orderedCellList?.map((cell) => (
                  <CellMeetingCellStaticTableRow
                    key={cell.id}
                    cellId={cell.id}
                    cellName={cell.name}
                    from={dateRagnge.from!}
                    to={dateRagnge.to!}
                  />
                ))
              ) : (
