...
interface CreateCellStepsFinalProps {}

const CreateCellStepsFinal = ({}: CreateCellStepsFinalProps) => {
  const queryClient = useQueryClient();
  const [createCellInfo, setCreateCellInfo] = useRecoilState(createCellState);
  const [disabled, setDisabled] = useState(false);
  const { mutateAsync, isError, isSuccess, data } = useCreateCellMutation<
    CreateCellMutation,
    CreateCellMutationVariables
  >(graphlqlRequestClient, {
    ...
    onError(error) {
      if (error instanceof Error) {
        toast.error(`셀 생성에 실패했습니다.`);
      }
    },
  });

  const { isLoading, data: cells } = useFindCellsQuery<
    FindCellsQuery,
    FindCellsQueryVariables
  >(
    graphlqlRequestClient,
  ...
  const onCreateHandler = () => {
    ...
    mutateAsync({
      input: {
        ...
        cellLeaderId: createCellInfo.leader.id,
      },
    });
  };
  ...
  return (
    <div className="mt-14 mb-6">
      <p className="text-sm text-center">
        생성하기 버튼을 눌러 셀을 생성해주세요
        <br />
        생성 후 결과를 확인할 때까지 팝업창을 닫지 마세요!
      </p>
      <div className="mt-8">
        {isLoading && (
          <div className="flex">
        ...
        {isSuccess ? (
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-lg text-center mt-3">
              <strong>{data.createCell.cell.name}</strong>을 성공적으로
              생성하였습니다
            </p>
          </div>
        ) : (
...
export default CreateCellStepsFinal;
