...
const EvaluationCommunitySection = ({ seasonName, communityName }: { communityName: string; seasonName: string; }) => {
  ...
  const [ inProgressSubmissionsCount, setInProgressSubmissionsCount ] = useState<number>(0)
  ...
  useEffect(() => {
    if (submissionCheckData && communityCellList) {
      ...
      for (let i = 0; i < submissionCheckData.length; i++) {
        const isInCommunityCellList = communityCellList.some(cell => cell.id === submissionCheckData[i].cellId)
        if (isInCommunityCellList) {
          if (submissionCheckData[i].submissionStatus === EvaluationSubmissionStatus.COMPLETE) {
            tempCompleteCount++ 
          } else if (submissionCheckData[i].submissionStatus === EvaluationSubmissionStatus.INPROGRESS) {
            tempInProgressCount++
          } 
        }
      }
                  ...
                  {completeSubmissionsCount !== 0 && communityLength !== 0 && completeSubmissionsCount === communityLength ? (
                    ...
                    <div className='flex justify-evenly'>
                      ...
                      <span className='inline-flex items-center  bg-gray-50 px-2 py-1 text-sm font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10 cursor-not-allowed'>
                        미제출 : {communityLength - completeSubmissionsCount - inProgressSubmissionsCount}셀
                      </span>
                    </div>
                  )}
                </div>
