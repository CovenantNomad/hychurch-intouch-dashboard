...
type SearchAndSelectModalProps = {
  open: boolean;
  query: string;
                  ...
                  >
                    {filteredPeople.map((person) => (
                      <Combobox.Option
                        ...
                        value={person}
                        className={
                          "flex justify-between items-center cursor-pointer select-none px-4 py-2 hover:bg-indigo-600 hover:text-white"
                        }
                      >
                        <span>{person.name}</span>
                        <button 
                          onClick={() => {
                            setSearchedMember(currentList => {
                              if (currentList === null) {
                                return [person];
                              }
                              return [...currentList, person];
                            })
                          }} 
