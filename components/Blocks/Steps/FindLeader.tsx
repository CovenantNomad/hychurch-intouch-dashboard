...
interface FindLeaderProps {
  leader: Leader
  setLeader: Dispatch<SetStateAction<Leader>>
  viceLeader: Leader
  setViceLeader: Dispatch<SetStateAction<Leader>>
...
const FindLeader = ({ leader, setLeader, viceLeader, setViceLeader, setCellName }: FindLeaderProps) => {
  ...
  const onSearchHandler = ({ name }: FindLeaderForm) => {
    setSearch(name)
  }

  const onSetLeaderHandler = ({id, name}: Leader) => {
    setLeader({
      id,
      name,
    })
  ...
  const onSetViceLeaderHandler = ({id, name}: Leader) => {
    setViceLeader({
      id,
      name
    })
  ...
  return (
    <div className="mt-14 mb-6">
      {!leader.name && <p className="text-sm text-red-600 text-center mb-2">셀리더를 선택해야 다음단계로 넘어갈 수 있습니다</p>}
      <div className="border border-gray-300 divide-y-[1px] mb-6 rounded-md">
        ...
        <div className="grid grid-cols-3 px-4 py-2">
          ...
          <div className="col-span-2 flex justify-between">
            <p className={`${viceLeader.name ? "text-black" : "text-gray-400"}`}>{viceLeader.name}</p>
            {viceLeader.name && (
      ...
      <form onSubmit={handleSubmit(onSearchHandler)}>
        <div className="border border-gray-200 rounded-2xl flex items-center bg-white px-4 py-1 max-w-xl">
          <label htmlFor="search" className="sr-only">셀리더 검색</label>
      ...
      <div className="mt-4">
        <table className="w-full border border-gray-300">
          <thead className="grid grid-cols-4 border-b border-b-gray-300">
