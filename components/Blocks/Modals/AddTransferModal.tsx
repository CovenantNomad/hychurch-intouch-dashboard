...
import { cellInfoTypes, Selected, Step, TransferInfo } from '../../../interface/cell';
import Stepper from '../Steps/Stepper';
import SelectMember from '../../Screens/Cells/Transfer/ModalSteps/SelectMember';
import SelectTransferCell from '../../Screens/Cells/Transfer/ModalSteps/SelectTransferCell';
import ConfirmStage from '../../Screens/Cells/Transfer/ModalSteps/ConfirmStage';
...
const AddTransferModal = ({ setModalOpen }: AddTransferModalProps) => {
  ...
  return (
    <Backdrop onClick={onCloseHandler}>
      ...
      >
        <button onClick={onCloseHandler} className="text-sm font-bold hover:text-teal-600 uppercase p-2">Close</button>

        <div className='container mt-5 flex flex-col items-center'>
          <h5 className='text-2xl font-bold mb-1'>셀원 이동하기</h5>
          <span className='text-sm text-gray-600'>이동할 셀원과 이동할 셀을 선택해주세요</span>
          <div className='w-[80%] my-6'>
            <Stepper currentStage={currentStage} steps={steps} setSteps={setSteps}/>
            {stages[currentStage-1].component}
          </div>

          <StepControl onNextHandler={onNextHandler} onBackHandler={onBackHandler} currentStage={currentStage} user={{id: transferInfo?.user.userId || "", name: transferInfo?.user.name || ""}} lastStage={TOTAL_STAGE} />
        </div>
      </motion.div>
    </Backdrop>
  );
