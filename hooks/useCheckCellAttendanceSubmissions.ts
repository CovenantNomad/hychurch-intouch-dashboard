...
import { CellLeaderAttendanceSubmissionStatus, CheckCellAttendanceSubmissionsQuery, CheckCellAttendanceSubmissionsQueryVariables, useCheckCellAttendanceSubmissionsQuery } from "../graphql/generated";
...
const useCheckCellAttendanceSubmissions = ( attendanceDate: string ) => {
  ...
  const [ communityWay, setCommunityWay ] = useState<AttendanceSubmissionType[] | null>(null)
  ...
  useEffect(() => {
    if (!isDataLoading && !isFetching) {
      ...
      if (data && data.cellAttendanceCheckSubmissions) {
        ...
        const commonCell = data.cellAttendanceCheckSubmissions.filter((cell) =>
        ...
        if (commonCell.filter(cell => cell.submissionStatus !== CellLeaderAttendanceSubmissionStatus.Complete).length === 0) {
